...
const CreateMetricComponent = ({ metricId, valueType }) => {
  ...
  const [createMetricEntry, { data, loading, error }] = useMutation(ValueTypeMutations[valueType], { client: Client });
  ...
  const handleCreateMetricEntry = () => {
    ...
    createMetricEntry({ variables: { metricId, type: valueType, value, timestamp: t } });
  };

  return (
    <div className="create-metric-entry">
      <Input
        label="Entry Value"
        value={value}
      ...
      />
      <Button onClick={handleCreateMetricEntry} disabled={loading}>
        {loading ? 'Creating...' : 'Create Metric Entry'}
      </Button>
      {data?.createMetricValue?.value && <p>Metric Entry created with {data.createMetricValue.value.value} at {data.createMetricValue.value.timestamp}</p>}
      {data?.createMetricValue?.errors[0] && <p>Errors: {data.createMetricValue.errors}</p>}
